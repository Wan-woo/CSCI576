...
class Stitcher:
    ...
    def stitch(self, images, ratio=0.75, reprojThresh=4.0,showMatches=False, imageIndex=0):
        print("entering: ",imageIndex)
        ...
        cv2.imwrite("output/frame_"+str(imageIndex)+".jpg", result[0:imageB.shape[0],0:imageB.shape[1]])
    ...
    def reverseStitch(self, images, ratio=0.75, reprojThresh=4.0,showMatches=False, imageIndex=0):
        print("entering: ",imageIndex)
        # unpack the images, then detect keypoints and extract
        # local invariant descriptors from them
        (imageB, imageA) = images
        ...
        offsety = abs(int(f1[1]))
        offsetx = abs(int(f1[0]))
        dsize = (max(int(ds[0])+offsetx, imageA.shape[1]+offsetx), max(int(ds[1]) + offsety,imageA.shape[0]+offsety))
        print("image dsize =>", dsize)
        print("entering warpPerspective")
        result = cv2.warpPerspective(imageB, xh, dsize)
        #tmp[0:dsize[0], historyOffsetX:dsize[1]] = [0,0,0]
        #tmp[historyOffsetY:b.shape[0]+historyOffsetY, historyOffsetX:b.shape[1]+historyOffsetX] = b
        #result[offsety:imageA.shape[0]+offsety, offsetx:imageA.shape[1]+offsetx] = imageA
        print("done warpPerspective")
        #cv2.imshow("result", result)
        #cv2.waitKey()
        #result[0:imageB.shape[0], 0:imageB.shape[1]] = imageB
        for i in range(imageA.shape[0]):
            for j in range(imageA.shape[1]):
                if (imageA[i,j][0] != 0 or imageA[i,j][1] != 0 or imageA[i,j][2]) != 0:
                    result[i+offsety,j+offsetx] = imageA[i,j]
        cv2.imwrite("output/frame_"+str(imageIndex)+".jpg", result[offsety:imageA.shape[0]+offsety, offsetx:imageA.shape[1]+offsetx])
        #result = cv2.warpPerspective(imageB, H,
        #(imageB.shape[1] + imageA.shape[1], imageB.shape[0]))
        #result[0:imageA.shape[0], 0:imageA.shape[1]] = imageA
